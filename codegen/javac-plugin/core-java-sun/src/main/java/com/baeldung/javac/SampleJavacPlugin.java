...
public class SampleJavacPlugin implements Plugin {
                ...
                e.getCompilationUnit()
                        .accept(new TreeScanner<Void, Void>() {
                            ...
                            public Void visitClass(ClassTree node, Void v) {
                                ...
                                if (node.getModifiers().getAnnotations().stream().anyMatch(a -> Positive.class.getSimpleName().equals(a.getAnnotationType().toString()))) {
                                    JCTree.JCVariableDecl variable = createVariable(context, node);

                                    Log.instance(context)
                                            .printRawLines(Log.WriterKind.NOTICE, "variable=" + variable);


                                    JCTree.JCClassDecl classDecl = (JCTree.JCClassDecl)node;
                                    classDecl.defs = classDecl.defs.append(variable);
                                }
    ...
    private static JCTree.JCVariableDecl createVariable(Context context, ClassTree node) {
        ...
        JCTree.JCModifiers modifiers = factory.Modifiers(Flags.PRIVATE);

        Name variableName = symbolsTable.fromString("variable");


        //Name stringType = symbolsTable.fromString("java.lang.StringBuilder");



        JCTree.JCExpression type = factory.Ident(symbolsTable.fromString("StringBuilder"));

        JCTree.JCExpression value = null;//factory.NewClass() ClassLiteral(symtab.stringBuilderType); //Literal("hi man!");

        //JCTree.JCPrimitiveTypeTree type = factory.TypeIdent(TypeTag.INT);
        //JCTree.JCExpression value = factory.Literal(new Integer(1));



        return factory.at(((JCTree) node).pos).VarDef(modifiers, variableName, type, value);
    }
